---
title: Getting into Plotly with Gapminder
author: Alex Michel
date: '2018-06-20'
slug: getting-into-plotly-with-gapminder
categories:
  - blog
tags:
  - gapminder
...
I feel like a graph like this could be interesting as an educational tool. I've seen similar plots on the OECD website. So the audience could be the general public, but particularly people involved in international development, public health, and equity.
...
So I made a really dumb error at the beginning of graphic this plot. I couldn't get some really basic filtering to work and I could not figure out why. It turned out that there just wasn't any data for the years I had initially selected. Once I switched years I was successfully able to get some data to start plotting. 

For the lab day to work on our plots, Ryan gave me some help by showing me Plotly. He also explained what a slope graph was so that I could look up some examples of recipes for slope graphs.
...
# I know this is not Tidy but I kept getting weird errors and I wasn't sure what to do
highlighting2 <- highlighting %>% 
  mutate(change_LE = `2007_LE` - `1952_LE`) %>% 
  mutate(date = seq(1866, 2007, by= 1)) %>% 
  mutate(Life_exp = seq(1, 142, by = 1))
